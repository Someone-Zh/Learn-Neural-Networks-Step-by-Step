...
functions = [
    {
        ...
        'derivative': lambda x: np.full_like(x, 2),  # 修复：返回数组而非标量
...
for func in functions:
    ...
    deriv_line, = ax.plot([], [], color=func['color'], linestyle='--', linewidth=2, label=f'导数 {func["name"].split(" ")[1]}')
...
# 可选：保存动画为MP4（需安装ffmpeg）
# ani.save('导数推理动画.mp4', writer='ffmpeg', fps=30, dpi=100)
