...
def load_mnist_images(filename):
    with gzip.open(filename, 'rb') as f:
        ...
        # 使用NumPy高效处理
        # 将字节流转换为NumPy数组，然后归一化
        images = np.frombuffer(raw_data, dtype=np.uint8).reshape(num_images, rows * cols)
        return images
...
class MnistModel():
    ...
    def train_step(self, X_data, z_data, lr, optim_class):
        ...
        batch_size = len(z_data)
        z_true_data = np.zeros((batch_size, 10), dtype=np.float64)
        z_true_data[np.arange(batch_size), z_data.astype(int)] = 1.0
    ...
    def get_params(self):
        ...
        return {
            'layer1': {
                'W': self.layer1.W.data.tolist(),
                'b': self.layer1.b.data.tolist()
            },
            'layer2': {
                'W': self.layer2.W.data.tolist(),
                'b': self.layer2.b.data.tolist()
            },
            'layer3': {
                'W': self.layer3.W.data.tolist(),
                'b': self.layer3.b.data.tolist()
            }
        }
    ...
    def load_params(self, params):
        ...
        参数:
            params: 包含权重和偏置的字典
        """
        self.layer1.W.data = np.array(params['layer1']['W'], dtype=np.float64)
        self.layer1.b.data = np.array(params['layer1']['b'], dtype=np.float64)
        self.layer2.W.data = np.array(params['layer2']['W'], dtype=np.float64)
        self.layer2.b.data = np.array(params['layer2']['b'], dtype=np.float64)
        self.layer3.W.data = np.array(params['layer3']['W'], dtype=np.float64)
        self.layer3.b.data = np.array(params['layer3']['b'], dtype=np.float64)
    ...
    def get_grads(self):
        ...
        return {
            'layer1': {
                'W': self.layer1.W.grad.copy(),
                'b': self.layer1.b.grad.copy()
            },
            'layer2': {
                'W': self.layer2.W.grad.copy(),
                'b': self.layer2.b.grad.copy()
            },
            'layer3': {
                'W': self.layer3.W.grad.copy(),
                'b': self.layer3.b.grad.copy()
            }
        }
...
def train():
    ...
    # 保存模型参数
    model.save('3.2_工程化加速/mnist_model.json')
...
def _train_batch_worker(args):
    ...
    z_true_data = np.zeros((batch_size, 10), dtype=np.float64)
    z_true_data[np.arange(batch_size), y_batch.astype(int)] = 1.0
    z_true = Tensor(z_true_data)
...
def train_parallel(num_workers=4, batch_size=32, learning_rate=0.2, epochs=2):
    """并行训练MNIST模型（数据并行）
            ...
            for grads, loss in results:
                ...
                if avg_grads is None:
                    ...
                    for layer_name in ['layer1', 'layer2', 'layer3']:
                        avg_grads[layer_name]['W'] += grads[layer_name]['W']
                        avg_grads[layer_name]['b'] += grads[layer_name]['b']
            
            # 平均梯度
            num_results = len(results)
            if num_results > 0:
                for layer_name in ['layer1', 'layer2', 'layer3']:
                    avg_grads[layer_name]['W'] /= num_results
                    avg_grads[layer_name]['b'] /= num_results
...
def visualize_predictions(model_path='mnist_model.json', num_samples=10):
    ...
    # 预测
    x_samples = x_test_normalized[indices]
    ...
    for i, idx in enumerate(indices):
        ax = axes[i // 5, i % 5]
        matrix = x_test[idx].reshape(28, 28)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if mode == 'train':
            ...
            model_path = sys.argv[2] if len(sys.argv) > 2 else '3.2_工程化加速/mnist_model.json'
            test(model_path)
        elif mode == 'visualize':
            model_path = sys.argv[2] if len(sys.argv) > 2 else '3.2_工程化加速/mnist_model.json'
            ...
            print("用法:")
            print("mnist.py train              # 串行训练模型")
            print("mnist.py train_parallel [N] # 并行训练模型 (N为进程数，默认4)")
            print("mnist.py test [model]       # 测试模型")
            print("mnist.py visualize [model]  # 可视化预测结果")
    else:
        # 默认执行训练
        train()